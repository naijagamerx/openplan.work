...
// Android APK Release Detection
$apkDownloadUrl = '';
$hasApkRelease = false;
$apkArtifactsDir = ROOT_PATH . '/release-artifacts/android';
if (is_dir($apkArtifactsDir)) {
    $apkCandidates = glob($apkArtifactsDir . '/*.apk');
    if (is_array($apkCandidates) && !empty($apkCandidates)) {
        usort($apkCandidates, static function(string $a, string $b): int {
            return filemtime($b) <=> filemtime($a);
        });
        $latestApk = $apkCandidates[0];
        $apkDownloadUrl = APP_URL . '/release-artifacts/android/' . basename($latestApk);
        $hasApkRelease = true;
    }
}
...
<?php if ($hasApkRelease): ?>
<a class="w-full sm:w-auto px-6 py-3.5 bg-zinc-800 text-white rounded-xl font-bold flex items-center justify-center gap-2 hover:bg-zinc-700 transition-all border border-zinc-700" href="<?php echo e($apkDownloadUrl); ?>">
<svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
    <path d="M17.523 15.341l-1.34-1.34A6.963 6.963 0 0012 5a6.963 6.963 0 00-4.183 8.999l-1.34 1.34l1.341 1.341l1.34-1.34A6.963 6.963 0 0012 19a6.963 6.963 0 004.183-8.999l1.34-1.34l-1.341-1.341z"/>
</svg>
                        Android
                    </a>
<?php endif; ?>

<?php if (!$hasHostedRelease && !$hasApkRelease): ?>
